...
def bootstrapTable(request):
    return render(request, 'showcase/bootstrapTable.html')
...
def getdata(request):
    ...
    # db = pymysql.connect("192.168.207.160", "root", "123qwe!@#", "autotest", charset='utf8')
    db = pymysql.connect("localhost", "root", "guchen", "autotest", charset='utf8')

    cursor = db.cursor()
    rows = []
    # 根据是否存在搜索关键字执行不同sql,用来返回符合条件的数据
    if search_kw:
        # saasCount = "select id,total,succ,fail,percent from saas where id like '%%%s%%' or total like '%%%s%%'" % (search_kw,search_kw)
        saasCount = "select project_name,project_code,description,project_manager from project " \
                    "where project_name like '%%%s%%' or description like '%%%s%%' " \
                    "or project_manager like '%%%s%%'" % (search_kw, search_kw, search_kw)
    else:
        saasCount = "select project_name,project_code,description,project_manager from project"
    cursor.execute(saasCount)
    saas_results = cursor.fetchall()
    ...
    for i in list(saas_results):
        ...
        rows.append({"project_name": i[0], "project_code": i[1], "description": i[2], "project_manager": i[3]})
    print rows
    # rows返回为json数组
